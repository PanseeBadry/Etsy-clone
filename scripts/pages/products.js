...
function displayCategory() {
    ...
    if(categoryID != 0 && subcategoryID ==0){
        ...
        category = categoryID == 0 ? cats.find(c => c.id == foundSubCat.category_id) : cats.find(c => c.id == categoryID)
    }
     else if (checkFav) {
...
function displaySubCategories() {
    ...
    subcatContainer.classList.toggle('sub-category-min', displayedSubCats.length < 6)
...
function displayProducts() {
    ...
    showMoreBtn1.classList.toggle("hide", displayedProducts.length <= 4 && displayedProducts.length >= 0);
...
function displayProduct(product, len) {
    if (currentProductIndex <= len - 1) {
        ...
        favIcon.className = `fa-heart product-favorite ${Helper.isFav(product.product_id) ? "red fa-solid" : "fa-regular"
            } `
        ...
        favIcon.addEventListener('click',function (){
            if(checkFav && favIcon.classList.contains('fa-regular')){location.reload()}
